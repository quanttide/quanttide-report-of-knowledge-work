...
个人课程草稿与课程研发档案的比对，2026-09-10 走「compare-course-profile」这条工作流：找齐两边、逐项对照、给出处置结论。下面是精简后的正文。

## 定位

个人草稿在 `data/profile/iGuo/materials/course/`，3 篇 Markdown（`index.md` 量潮课堂目录、`knowledge-work.md` 知识工作、`production-internship.md` 生产实习），单文件口语叙述，无课时编号、无 JSON、无验收标准。

课程研发档案在 `../quanttide-course/data/profile/`，自述为课程内容的唯一创作源头：每门课一目录，每课时一目录（`lessonN-<slug>/`），`index.md` 与 `index.json` 成对，场景文件另有 `0X-<scene>.*`。

## 主诊断

两边只在「生产实习」这个课名上重叠，内容层几乎不交叠——草稿停在构思与制度（指导边界、壳即 baseline、观察不纠正），档案停在交付格式（课时目录、双格式、验收标准），草稿里能课程化的部分一处也没落成课时。

## 比对

**口径**：草稿以「量潮课堂」为验证组织的产品构思——用体系教体系，课程是验证产出质量是否可见提升的手段；档案是以生产实习为中心的课程内容源头，口径是教学交付。

**重叠**：只在「生产实习」这个课名上。草稿给的是制度与验证思路（指导边界、壳即 baseline、观察不纠正）；档案的 production-internship 只有 `lesson1-second-brain`（用第二大脑开工作区、逐级提交），README 还是占位。知识工作在档案里没有对应课程；vibe-coding 档案三课时齐全（zed / feishu / github），草稿未提及；data-engineering 档案只有需求（理论 24 学时、实验 8 学时），无课时，草稿也未提及。
...
**是什么**：同一领域的两份课程资料——个人草稿写构思与制度，课程研发档案是唯一创作源头、写课时与交付。

**问题在哪**：内容层几乎不交叠；草稿里能课程化的部分没有课时形态，档案里该有的课时与验收还缺着。

**先改什么**：把草稿里能课程化的部分回流成课时——「知识工作」方法论课先落地，生产实习的制度与 baseline 观察法收进课时与课程说明。

一句话复述：两边只在课名上碰了面，要动的是把草稿的构思落成档案的课时。
...
**不并**：「量潮课堂」验证组织本身——验证目标、风险、回报机制，属验证组织与主体层的设计。
